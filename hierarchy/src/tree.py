...
class Tree(object):
    ...
    def get_children_on_tree(self, node: Node):
        children_all = node.get_children()
        children_is_on_tree = [self.is_node_on_tree(child) for child in children_all]
        return children_all[children_is_on_tree]

    def add_node_to_tree(self, node: Node):
        if not self._root.__eq__(node.get_root()):
            raise LookupError("cannot add node from other root")
        while not self.is_node_on_tree(node):
            node_parent = node.get_parent()
            if self.is_node_on_tree(node_parent):
                ...
                self.add_node_to_tree(node_parent)
        pass

    def drop_node_from_tree(self, node: Node):
        while self.is_node_on_tree(node):
            node_children_on_tree = self.get_children_on_tree(node)
